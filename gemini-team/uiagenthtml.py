...
def extract_privacy_controls() -> Dict[str, Any]:
    """Extract likely privacy/security/data controls on the page with rich metadata."""
    try:
        pg: Page = playwright_context.get("page")
        if not pg:
            return {"status":"error","message":"No active page"}
...
() => {
  ...
  const norm = (s) => (s || "").replace(/\s+/g, " ").trim();
  const getText = (el) => norm(el.innerText || el.getAttribute('aria-label') || el.getAttribute('title') || "");
  const byId = (el) => el.id ? `#${CSS.escape(el.id)}` : "";
  const cls = (el) => {
    if (!el.className || typeof el.className !== 'string') return "";
    const parts = el.className.trim().split(/\s+/).slice(0,3).map(x => `.${CSS.escape(x)}`);
    return parts.join("");
  };
  const tag = (el) => (el.tagName || "").toLowerCase();

  const selectorOf = (el) => {
    // Prefer stable selector: tag#id.class1.class2...
    const t = tag(el);
    return `${t}${byId(el)}${cls(el)}` || t || "unknown";
  };
  ...
  const isVisible = (el) => {
    ...
    return !!(rect.width && rect.height) && document.contains(el);
  };

  const textOfIds = (idsStr) => {
    if (!idsStr) return "";
    return idsStr.split(/\s+/).map(id => {
  ...
  const groupLabel = (el) => {
    // Fieldset/legend or aria-labelledby chain
    ...
    if (labelled) return textOfIds(labelled);
    // try nearest label
  ...
  const helpText = (el) => {
    ...
    if (described) return textOfIds(described);
    return "";
  };

  const switchState = (el) => {
    // Normalize to on/off/mixed/null
    const aria = el.getAttribute("aria-checked");
    if (aria === "true") return "on";
    if (aria === "false") return "off";
    if (aria === "mixed") return "mixed";
    // Fall back to checked when role isn't provided
    if ("checked" in el) return el.checked ? "on" : "off";
    return null;
  };

  const collectSelectMeta = (el) => {
    ...
    return {
      value: el.value ?? null,
      options: opts.slice(0, 500),
      selectedText: selected.map(s => s.label).slice(0, 50)
    };
  };
  ...
  const baseMeta = (el, type) => ({
    ...
    value: ("value" in el && typeof el.value !== "undefined") ? el.value : null,
    visible: isVisible(el),
    bbox: (() => {
      const r = el.getBoundingClientRect();
      return { x: Math.round(r.x), y: Math.round(r.y), width: Math.round(r.width), height: Math.round(r.height) };
    })(),
    groupLabel: groupLabel(el) || null,
    helpText: helpText(el) || null
  });
  ...
  document.querySelectorAll('[role="switch"]').forEach(el => {
    const meta = baseMeta(el, "toggle");
    meta.state = switchState(el);
    out.push(meta);
  });

  // Checkboxes
  document.querySelectorAll('input[type="checkbox"]').forEach(el => {
    const meta = baseMeta(el, "checkbox");
    meta.state = el.checked ? "on" : "off";
    out.push(meta);
  });

  // Radios
  document.querySelectorAll('input[type="radio"]').forEach(el => {
    const meta = baseMeta(el, "radio");
    meta.state = el.checked ? "on" : "off";
    out.push(meta);
  });
  ...
  document.querySelectorAll('select').forEach(el => {
    const meta = baseMeta(el, "select");
    Object.assign(meta, collectSelectMeta(el));
    out.push(meta);
  });

  // Buttons with privacy-ish text
  document.querySelectorAll('button').forEach(el => {
    const t = (el.innerText || "").toLowerCase();
    if (t.includes('privacy') || t.includes('record') || t.includes('consent') || t.includes('security') || t.includes('data')) {
      out.push(baseMeta(el, "button"));
    }
  });

  // Links with privacy-ish text
  document.querySelectorAll('a').forEach(el => {
    const t = (el.innerText || "").toLowerCase();
    if (t.includes('privacy') || t.includes('cookie') || t.includes('security') || t.includes('data') || t.includes('record')) {
      out.push(baseMeta(el, "link"));
    }
  });

  // Filter by privacy keywords on label/text to reduce noise
  const KW = [
    "privacy","data","security","consent","record","retention","gdpr","ccpa","cookie","cookies",
    "tracking","telemetry","ads","advertising","personalization","permissions","sharing",
    "third-party","third party","download data","export data","delete account","deactivate",
    "identity","two-factor","2fa","mfa","password","profile visibility","who can see",
    "camera","microphone","screen sharing","meeting","transcript","caption","ai"
  ];

  const matchesKw = (s) => {
    const x = (s || "").toLowerCase();
    return KW.some(k => x.includes(k));
  };

  const filtered = out.filter(c =>
    matchesKw(c.label) ||
    matchesKw(c.helpText) ||
    matchesKw(c.groupLabel)
  );
...
""")
        ...
        # No post-filtering needed; result already keyword-filtered & enriched.
        return {"status":"success","controls": result}
...
def crawl_privacy_map(max_depth: int = 3, max_pages: int = 25, same_origin_only: bool = True) -> Dict[str, Any]:
    ...
    - Prioritize links with privacy keywords
    - Bound by depth/pages; optional same-origin guard
    - Record paths and controls; count clicks
    """
    try:
        ...
        visited: Set[str] = set()
        queue: List[Tuple[str, List[Dict[str,str]]]] = [(start_url, [])]  # (url, path: list of {action, label, href/selector})
        ...
        while queue and page_count < max_pages:
            ...
            try:
                expand_privacy_sections(max_clicks=int(os.environ.get("CRAWL_EXPAND_CLICKS","8")))
            except Exception:
                pass

            # 2) First scrape controls on the *fully expanded* page
            ...
            # 3) Collect candidate links and rank them; prefer staying in settings
            snap = dom_snapshot()
            if snap.get("status") != "success":
                continue
            anchors = snap["dom"].get("anchors", [])
            ...
            for a in anchors:
                ...
                if same_origin_only and not _same_origin(abs_url, start_host):
                    continue
                # Skip denied links (e.g., accessibility/legal/etc.)
                if _is_denied_link(href, text):
                    continue
            ...
            # 4) Enqueue high-priority first, then a *small* sample of others
            def next_steps(candidates: List[Dict[str,str]], cap: int) -> List[Dict[str,str]]:
                return candidates[:cap]

            # caps are tunable via env
            cap_prio = int(os.environ.get("CRAWL_CAP_PRIO","8"))
            cap_other = int(os.environ.get("CRAWL_CAP_OTHER","2"))

            for cand in next_steps(prio_scored, cap_prio) + next_steps(other_scored, cap_other):
                if cand["abs"] in visited:
                    continue
                ...
                queue.append((cand["abs"], new_path))
        ...
        report = {
            ...
            "summary": {
                "total_clicks": total_clicks,   # We navigated by goto; set to len(path)-like if you choose to “real click”
...
Secondary Objective (optional if site has meeting/conference launching flow)
-Meeting Mode (generalized, browser-only)
- After finishing settings scraping, attempt to start/host/join a meeting IN THE BROWSER (never the desktop app).
- If a page or modal proposes opening a desktop app, ALWAYS choose the browser path:
  Preferred labels/links (try in this order):
    "Join from your browser" • "Continue in browser" • "Use web client" • "Join on web" •
    "Join from Web" • "Launch web client" • "Join using browser"
  Decline/close examples:
    "Open [App]?" • "Open Zoom Meetings?" • "Open Microsoft Teams?" • "Open Webex?" •
    "Open the desktop app" → click "Cancel" / "Not now" / "X" / "Stay on web"
- Never click custom-protocol links (e.g., zoommtg:, msteams:, webex:, lync:, sip:, tel:). If present, ignore and continue looking for a web link.
- If the web client opens in a NEW tab, use tabs_switch_to to focus it.
- Wait for the meeting UI to be ready before scraping (allow page to settle):
  • Wait for a visible control strip or meeting canvas (e.g., buttons containing: "Mute", "Start Video", "Participants", "Share", "Security", "More").
  • Use a short grace period (1–3 s) after the UI appears to allow dynamic elements to load.
- Once ready, prefer DOM scraping (extract_privacy_controls) over screenshots. Focus on privacy/security/consent elements:
  • Recording consent / “This meeting is being recorded” banners
  • Security/Privacy menus, Participant permissions, Waiting Room, Chat/file sharing toggles
  • Caption/transcript/AI features, screen-sharing restrictions
- Do not navigate away from the meeting until after scraping these elements.
...
-- Produce a JSON report with:
   ...
   - discoveries: for each page with relevant controls, include `path` (sequence of clicks with labels/hrefs) and `controls` (label, type, selector)
   - summary counts
- Once completing the primary objective (and secondary objective if available), save the report and end the session. 
