...
## Overview
This repository builds an end-to-end pipeline that captures privacy/data settings from web platforms, normalizes them into a settings database, and drives an agentic UI that can change settings on a user's behalf. It solves the "every platform has a different UI" problem by combining Playwright automation with Gemini planning/verification and a reusable authenticated profile cache. This repository also includes an interactive dashboard that visualizes privacy settings across platforms using treemaps and comparative charts to help users understand the the structure and accessibility of privacy controls. 
...
## Privacy Settings Visualization Dashboard

In addition to the automated agent, this project includes an interactive visualization dashboard that helps users explore and understand privacy settings across platforms.

The dashboard is built using **D3.js** and visualizes the structured settings dataset generated by the crawler and classifier pipeline.

### Dashboard Features

The central visualization is a **treemap** that organizes privacy settings hierarchically:

Platform → Category → Individual Setting

Each rectangle represents a specific privacy setting extracted from the platform interface.

Treemap features include:

- **Platform grouping** – Settings are grouped by platform (e.g., Google, Facebook, LinkedIn).
- **Category classification** – Settings are categorized into standardized privacy themes such as:
  - security_authentication
  - data_collection_tracking
  - data_sharing_third_parties
  - visibility_audience
- **Interactive tooltips** – Hovering over a setting reveals the setting name, description, and current state.
- **Multiple metrics** – The treemap can display settings based on:
  - **Count** – total number of settings
  - **Click depth** – how many navigation steps are required to reach a setting
- **Comparative analysis** – Users can compare platforms to understand how privacy controls are distributed.

The dashboard allows users to visually assess:

- how complex privacy settings are on each platform
- which platforms expose more configurable settings
- which settings are harder to reach in the interface
...
- Milestone: Visualization dashboard
  - Code: `PrivacyAgentDashboard/explore/treemap.js`
...
- Expand visualization dashboard with additional comparative views.
- Support historical comparison of privacy settings over time. 
